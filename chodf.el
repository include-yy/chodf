...
;; inspired by jsdeferred, deferred.el, mmdeferred
...
(define-error 'chodf-error
  "error generated by chodf")
...
(defun chodf-ng (x)
  ...
  (signal 'chodf-error x))

(cl-defstruct (chodf (:constructor chodf-new)
		     (:copier nil))
  "data struct used to reprensent a deferred object.
okcb is the success callback and ngcb is the fail callback.
n means next, points to the next deferred object or nil"
  (okcb 'chodf-ok)
  (ngcb 'chodf-ng)
  n)
...
(defun chodf-error (odf fun)
  "Create new deferred and sets FUN as its err callback.
if FUN not signal 'chodf-error but just return normal value,
Deferred treats the give error is recovery and continue chain"
  (chodf--post odf :ng fun))
(defun chodf-ner (odf okfn ngfn)
  "Create new deferred and sets okcb to OKFN, ngcb to NGFN.
...
If 'chodf-error is signaled, the next path will be `:ng'"
  (let ((next :ok))
    (condition-case err
	(pcase okng
...
(defun chodf-callx (fun &rest args)
  "calls function asynchronous.
a short hand for `chodf-nextx'"
...
(defun chodf-parallel (&rest fun-or-d-ls)
  "parallel wraps up deferredlist to on deferred.
...
This is useful when some asynchronous resources are required."
  (let* ((ret (chodf-new))
    ...
    (seq-do-indexed
	 ...
	 (chodf-error it (lambda (e)
			   (chodf-fail ret e))))
       (cl-incf num))
     fun-or-d-ls)
    (when (= num 0) (chodf-nextx (lambda (_) (chodf-call ret []))))
    ret))

(defun chodf-chain (&rest funs)
  ...
  (let ((chain (chodf-nextx))
    ...
    (mapc (lambda (obj)
	    (cond
	     ...
	     ((and flag (functionp obj))
	      (setq chain (chodf-error chain obj)
		    flag nil))
	     ((functionp obj)
 ...
 * This function waits a deferred returned by loop function."
  (let ((i 0) it ret)
    (setq it (lambda (_)
		 ...
		 (if (chodf-p ret)
		     (chodf-next ret (lambda (r)
				       (setq ret r)
				       (chodf-nextx it)))
		   (chodf-nextx it)))))
    (when (> n i) (chodf-nextx it))))
